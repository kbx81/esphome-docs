...
encourage all developers to migrate their custom components to :doc:`/docs/components/external_components`. Please see
...
difference, but this change could result in some issues, particularly for :doc:`/docs/components/external_components`. You
...
:doc:`/docs/components/voice_assistant`. These components extend our work to help you create the ultimate personal voice
assistant hardware.

The new :doc:`speaker media player </docs/components/media_player/speaker>` component adds several features for building
a well-rounded audio device. It supports playing two different streams of audio: one for announcements and another
for music.

The new :doc:`mixer speaker </docs/components/speaker/mixer>` component lets you combine the two streams. The mixer
...
configuration variable in :doc:`/docs/components/image` and related components was renamed to ``transparency`` and changed
from a boolean to an enum; in addition, the ``type`` configuration variable is now mandatory. See the
:doc:`/docs/components/image` documentation for more detail.
...
Finally, the :doc:`/docs/components/online_image` was updated to add support for both BMP and JPEG image formats.

ILI9XXX Display Component
-------------------------

There are two changes to the :doc:`/docs/components/display/ili9xxx` component. Previously the component automatically loaded
