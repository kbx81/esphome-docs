...
:doc:`ESP32 Camera Support </docs/components/esp32_camera>`: This is huge. When integrating cameras
...
And thanks very much to :ghuser:`puuu` for the :doc:`SDS011 Particulate Matter integration </docs/components/sensor/sds011>` and
:ghuser:`mvturnho` for the newly added
:doc:`MPR121 Capacitive Touch sensor </docs/components/binary_sensor/mpr121>` support!
...
- The ``timeout_meter`` and ``timeout_time`` options of the ultrasonic sensor
  have been merged into ``timeout``. (:doc:`/docs/components/sensor/ultrasonic`)
...
- Improved validation. For small typos for config variables similar keys are automatically
  suggested.
- Added ``esp8266_restore_from_flash`` to :doc:`esphome section </docs/components/esphome>` which
...
- Rotary Encoders have a new ``min_value`` and ``max_value`` option. (:doc:`/docs/components/sensor/rotary_encoder`)
