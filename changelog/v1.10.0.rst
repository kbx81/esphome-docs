...
This release brings with it lots of goodies, but most important is the :doc:`native API </docs/components/api>`.
...
- The fastled effects have been renamed to ``addressable_`` for the new
  :doc:`Neopixelbus </docs/components/light/neopixelbus>` integration. See the validation error message
  for more info.
...
- Added :doc:`/docs/components/substitutions` to reduce repeating across configs.
...
- Added a bunch of guides (and helpers) for creating custom components. Also new: ``esphomeyaml.libraries``,
  ``esphomeyaml.includes`` and ``esphomeyaml.platformio_options`` (:doc:`/docs/components/esphome`)
...
- You can now use Home Assistant to get time in ESPHome, so no more need for SNTP. See :doc:`/docs/components/time/index`.
