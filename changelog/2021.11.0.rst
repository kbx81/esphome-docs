...
After we created :doc:`/docs/components/esp32_improv`, we thought it might be a good idea to implement the same for serial connections.
See the docs here for :doc:`Improv via Serial </docs/components/improv_serial>` and the
