...
The solution below handles the problem of RF frames being sent out by :doc:`/docs/components/rf_bridge` (or
:doc:`/docs/components/remote_transmitter`) too quickly one after another when operating radio controlled
...
In this example a :doc:`/docs/components/cover/time_based` is used with the GPIO configuration of a Sonoff Dual R2.
...
Sometimes it may be more confortable to use a :doc:`/docs/components/text/template` to change some numeric values from the user interface.
