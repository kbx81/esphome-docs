...
a :doc:`/docs/components/uart` component (and its requisite GPIO pins) is required in your device's configuration.
...
settings or read its state via the UART. A :doc:`/docs/components/binary_sensor/gpio` alone is sufficient if you only want
...
Multiple instances of this component may be defined if multiple :doc:`/docs/components/uart` components are available:
...
- **uart_id** (*Optional*, :ref:`config-id`): Manually specify the ID of the :doc:`/docs/components/uart` if you want
...
To determine presence, you can simply use a :doc:`/docs/components/binary_sensor/gpio` and connect a single GPIO pin
...
First, setup a :doc:`/docs/components/uart` and :ref:`dfrobot_sen0395-component` and then use its binary sensor platform
