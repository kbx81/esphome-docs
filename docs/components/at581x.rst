...
a :doc:`/docs/components/i2c` component (and its requisite GPIO pins) is required in your device's configuration.
...
A :doc:`/docs/components/binary_sensor/gpio` alone could be sufficient if you only want
...
Multiple instances of this component may be defined if multiple :doc:`/docs/components/i2c` components are available:
...
- **i2c_id** (*Optional*, :ref:`config-id`): Manually specify the ID of the :doc:`/docs/components/i2c` if you want
...
- Refer to other options from :ref:`Binary Sensor <config-binary_sensor>` and :doc:`GPIO Binary Sensor </docs/components/binary_sensor/gpio>`.
