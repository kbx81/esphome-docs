...
The ``mixer`` speaker platform allows you to mix audio sent to different source speakers into one output which is sent to another :doc:`speaker component </docs/components/speaker/index>`. Individual source speakers may be ducked (made quieter) with the :ref:`apply ducking action <mixer_speaker-apply_ducking>`.

When mixing multiple audio streams into one, they must have the same sample rate. If they are different, enable queue mode so that only one source speaker's audio is played at a time. Otherwise, use a :doc:`resampler speaker </docs/components/speaker/resampler>` to send audio to the source speakers.
...
- **output_speaker** (**Required**, :ref:`config-id`): The :doc:`speaker </docs/components/speaker/index>` to output the mixed audio.
