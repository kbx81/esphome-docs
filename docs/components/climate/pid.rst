...
- At least one :ref:`float output <config-output>` to drive for heating or cooling (or both).
  This could for example be a PWM output via :doc:`/docs/components/output/sigma_delta_output` or :doc:`/docs/components/output/slow_pwm` that drives a heating unit.
...
.. note::
    ...
    We recommend putting a filter on the sensor (see filters in :doc:`/docs/components/sensor/index`) and
...
2. Create a :doc:`template button </docs/components/button/template>` to start autotuning later:
...
- :doc:`/docs/components/output/sigma_delta_output`
