...
- ``esp32_improv`` as described in :doc:`/docs/components/esp32_improv`
- ``improv_serial`` as described in :doc:`/docs/components/improv_serial`, if a USB connection is available (recommended)
...
- Your project supports adoption via the ``dashboard_import`` feature of ESPHome (see
  :doc:`Sharing </docs/guides/creators>`). In particular:
