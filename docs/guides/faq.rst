...
- **ESP32-S3**
  ...
  - Has instruction set extensions which make it a better fit for applications which require some form of machine
    learning (:doc:`/docs/components/micro_wake_word`, for example).
...
- **ESP8266**
  ...
  - Use an ESP32-C3 when you're thinking you need to use an ESP8266 because the ESP32(-Sx) is "too powerful" or
    "overkill".
  - Does not meet the requirements of :doc:`/docs/guides/made_for_esphome`.
...
- *"...But the [ESP8266 board] is cheaper!!!"*...well, you get what you pay for. Compared with the ESP8266, all ESP32s
  ...
  - roughly 5x the amount of RAM.
    ...
    - Some components require more RAM than is available on the ESP8266 -- (large) displays and
      :doc:`some sensors</docs/components/sensor/bme68x_bsec2>` are known to regularly provoke issues/crashes on ESP8266s.
    - Workarounds are often available, but it's not reasonable to assume that a given workaround will work forever,
      ...
      :doc:`vendor-provided/maintained libraries for some functionality</docs/components/sensor/bme68x_bsec2>`.
...
- You can use :doc:`/docs/components/substitutions` to build on the examples above and reduce repetition in your
  configuration files.
...
- You can view the full list of command line interface options here: :doc:`/docs/guides/cli`
...
In some cases, community-provided :doc:`/docs/components/external_components` are available; keep in mind that these are not
...
- ESPHome intentionally reboots after a configured duration in specific situations, such as when the
  :doc:`Wi-Fi connection cannot be made </docs/components/wifi>`, :doc:`API connection is lost </docs/components/api>` or
  :doc:`MQTT connection is lost </docs/components/mqtt>`. To disable this behavior, you'll need to explicitly set the
...
Some of ESPHome's functionality relies on :doc:`mDNS</docs/components/mdns>`, so, naturally, disabling it will cause these
...
- Automatic discovery in Home Assistant when using the :doc:`native API </docs/components/api>` relies on mDNS broadcast
...
Important: follow these `instructions </docs/components/esphome.html#changing-esphome-node-name>`_ to use the
...
By leveraging the :doc:`/docs/components/external_components` feature, it's possible to test most pull requests by simply
...
The :doc:`Deep Sleep </docs/components/deep_sleep>` component needs to be present within your device's configuration when
